...
浏览器扩展版（`extension/`，Manifest V3）：无需安装桌面程序，在博看网页内直接下载。

### 新增

- **详情页一键下载**：在书刊详情页试读按钮旁注入「下载」按钮，点按弹出选项气泡——输出格式（自动 / PDF / EPUB）、图片压缩三档、「下载全年」（杂志类）
- **悬浮进度方块**：挂在页面右侧、可拖动记忆位置，站内跨页无缝同步；点击展开任务详情（逐期进度 / 失败原因 / 取消），完成后显示 √ / ×，仅留「取消 / 完成」操作
- **固定任务标签页**：下载引擎运行在常驻的 downloader 页（MV3 Service Worker 会空闲自杀，不能承载长任务）；「下载全年」入队后自动展开为逐期任务串行执行
- **纯 JS 合成流水线**：自写 PDF 生成器（JPEG 直嵌、两级章节书签、UTF-16BE 标题）；目录换算逻辑（页码偏移 / 同页去重 / 树形提升）与桌面版逐行对齐；图片压缩 worker 池并发（`min(4, CPU核数)`，对齐桌面版线程池）
- **扩展弹窗**：下载目录说明、章节书签开关、使用说明、关于、运行日志竖列收纳；主题与桌面版统一浅蓝紫渐变

### 修复（首批实测反馈）

- 「下载全年」静默退化为只下载当前期：任务展开阶段引用了尚未创建的下载控制器，异常被捕获后降级单本；现展开阶段自建控制器（收集各期时也可取消）
- 隐藏的下载标签页静置 5 分钟后浏览器将其定时器强节流，进度广播失联、悬浮方块停留在上一批完成态：进度上报改为事件驱动立即广播
- 新一批下载开始时自动清掉上一批完结记录，不再新旧混展

### 文档

- **README**：新增「浏览器扩展版」章节（安装方式、下载目录、批量下载防弹窗设置）
- **使用说明**：扩展弹窗内提示关闭浏览器「下载开始时显示下载菜单」，避免批量下载逐期弹窗
...
- **图片压缩多线程化**：参考安卓分支做法将压缩拆为单页函数 `_compress_one`，批量压缩改为线程池并行（`min(4, CPU核数, 页数)`）——PIL 解码/重采样/编码均释放 GIL，多核下接近线性提速；Windows 端保持「下载完 → 再压缩」的顺序流程，不引入下载/压缩并行
- **下载完成页精简**：移除底部「打开下载目录」按钮及相关 JS 逻辑，「返回主页面」整行占满；结果卡片内的「打开所在文件夹」保留

### 文档

- **README**：使用教程改为结果卡片「打开所在文件夹」，「输出目录」统一为「下载目录」，页脚更新至 2.1.0（本次随版本升 2.1.1）
- **使用说明.txt**：重写至 2.1.0（输入规则、下载全年、Calibre 元数据、缓存自愈），本次随版本升 2.1.1

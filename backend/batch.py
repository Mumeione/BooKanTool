...
def _run_pdf(
    ...
    dl_state = {"finished": False}

    def prog(done, total_, page):
        # 图片下载是耗时主体：占整体 2% ~ 90%（压缩在独立线程池并行消化）
        if total_ and done >= total_:
            dl_state["finished"] = True
        frac = 0.02 + 0.88 * (done / total_) if total_ else 0.02
        report(frac, f"下载图片 {done}/{total_}（第 {page} 页）")

    def cprog(done_c, total_c):
        # 下载收尾后剩余压缩任务收尾：占整体 90% ~ 91%。
        # 下载期间压缩回调不动进度条——否则进度会先跳到 90% 再回跳，
        # 且文字从"下载图片"切到"压缩"再切回，观感像抽搐。
        if not dl_state["finished"]:
            return
        frac = 0.90 + 0.01 * (done_c / total_c) if total_c else 0.90
        report(frac, f"正在压缩中（{done_c}/{total_c}）…")

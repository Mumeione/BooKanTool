...
(function () {
    ...
    app.on('ready', () => {
        log('info', '前端已就绪');
        // 版本号由后端注入（backend.config.APP_VERSION），避免多处硬编码
        if (window.__bookan_version) {
            $('footer-version').textContent = window.__bookan_version + ' · PyWebView';
        }

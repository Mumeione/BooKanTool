...
def user_data_dir() -> str:
    """~/BookanTool —— 默认下载目录与日志目录；安卓端为应用私有目录。"""
    try:
        from backend.config import android_base_dir, is_android

        path = (
            os.path.join(android_base_dir(), APP_NAME)
            if is_android()
            else os.path.join(os.path.expanduser("~"), APP_NAME)
        )
    except ImportError:
...
def run_selftest() -> None:
    ...
    # ── pywebview 模块可导入（不创建窗口也不启动 GUI；安卓端无 pywebview，跳过） ──
...
def _live_webview():
    """从当前 Activity 的 content view 取最上层的 WebView（pywebview
    的 setContentView 布局，child 0 即 WebView）。

    只能在可安全调 pyjnius 的线程使用：SDL 主线程，或生命周期回调
    所在的 UI 线程（Java→Python 桥自带 JNI 上下文）。"""
    from jnius import autoclass

    activity = autoclass("org.kivy.android.PythonActivity").mActivity
    if activity is None:
        return None
    content = activity.findViewById(0x01020002)  # android.R.id.content
    return content.getChildAt(0) if content is not None else None


def heal_webview_timers() -> None:
    """启动安全阀：无条件把 WebView 恢复到「运行中」状态。

    根因复盘（强杀重启后卡「正在握手后端」）：启动窗口期内 Activity 可能
    出现一次 spurious onPause，而生命周期钩子注册前的 onResume 早已错过——
    pauseTimers 把全局 JS timers 冻结后无人解冻。页面本体是原生加载不受
    影响，但所有 setTimeout/setInterval 停摆：握手轮询、超时 reload 全部
    死等，表现为「切走再切回才恢复」（on_resume 是唯一解冻路径）。

    对策：on_loaded（每次页面加载完成都触发）里 onResume + resumeTimers——
    二者对未暂停实例是幂等 no-op，对被冻结实例是解药。
    """
    try:
        wv = _live_webview()
        if wv is not None:
...
def patch_android_lifecycle() -> None:
    ...
    p4a 经 Application.ActivityLifecycleCallbacks（Java→Python 桥）把
    on_pause/on_resume 派发到 UI 线程；pywebview 原版实现持有的是启动时
    的 jobject 引用——Activity 重建后该引用指向已销毁的旧 WebView，
    resumeTimers 打在旧对象上无效，全局 timers 保持暂停 → 永远卡
    「正在握手后端」。

    修法：
      • on_pause / on_resume 每次都从当前 Activity 的 content view 重新
        获取 WebView 实例再操作（回调跑在 UI 线程，pyjnius 安全）；
    ...
    def on_pause(self, _activity):
        try:
            wv = _live_webview()
            if wv is not None:
                wv.onPause()  # 仅实例级暂停；全局 pauseTimers 一旦在启动
                # 窗口期被触发且无配对 resume，JS timers 永久冻结成死局
        except Exception:
            pass

    def on_resume(self, _activity):
        try:
            wv = _live_webview()
            if wv is not None:
                wv.onResume()
    ...
    def on_destroy(self, _activity):
        with contextlib.suppress(Exception):
            self.view.dismiss()  # pywebview 原清理逻辑，可能因引用失效抛异常
        # 无论划掉 / 返回 finish 还是旋转重建，进程一律随之结束：
        # 系统重建 Activity 时走全新启动，绝不残留半死进程
        _os._exit(0)

    AndroidApp.on_pause = on_pause
    AndroidApp.on_resume = on_resume
    AndroidApp.on_destroy = on_destroy
...
def main() -> None:
    ...
    parser.add_argument(
        "--web", action="store_true", help="浏览器联调模式：本地 HTTP 桥模拟安卓桥接（不启动 GUI）"
    )
    ...
    # ── 安卓端：pywebview Android 后端（p4a 环境自动启用，适配见 backend/android_compat.py） ──
    from backend.config import is_android

    android = is_android()
    if android:
        from backend.android_compat import setup_android_paths

        setup_android_paths()  # Android 无 /tmp，把流水线临时目录指到应用私有目录

    # ── 浏览器联调模式（--web）：HTTP 桥 + SSE，浏览器直接访问 ──
    if args.web:
        from backend.android_server import run_web_debug
    ...
    # ── 前置检查：WebView2 运行时（仅桌面 Windows） ──
    if not android and not args.no_gui_check:
    ...
    from backend.android_compat import AndroidBridge
    ...
    # ── 创建 API 桥接实例 ──
    # （安卓沿用 js_api 通道 —— 它从 WebView 的 Java 线程进入 Python，
    #   不触发 pyjnius 的 native 线程 attach，是已验证可靠的方向；
    #   唯一禁忌是后台线程调 evaluate_js 推事件，AndroidBridge._emit 已改为队列轮询）
    api = AndroidBridge() if android else BridgeAPI()
    ...
    if android:
        # Android 后端为全屏 WebView，窗口几何参数无意义，全部忽略
        pass
    else:
    ...
    def on_loaded():
        if android:
            ...
            from backend.android_compat import do_pending_main_actions

            do_pending_main_actions()  # 前端排队动作：拉起系统文件管理器
        try:
            if android:
                from backend.android_compat import default_output_dir as android_output_dir

                raw_dir = android_output_dir()
            else:
                raw_dir = default_output_dir()
            default_dir = raw_dir.replace("\\", "\\\\")  # JS 字符串转义
        except OSError:
        ...
        try:
            ...
            cfg_js = "{}"
        # 安卓：事件经 js_api 队列轮询回推（后台线程 evaluate_js 会 native 崩溃，
        # 见 android_compat.AndroidBridge._emit）。
        # 注意：pywebview 安卓后端每次 js_api 调用的返回值都经 pyjnius 从
        # UI 线程回传（ANR trace 实锤），轮询过密会打满 UI 线程导致
        # ANR —— 间隔不得低于 1s。
        poll_js = (
            "setInterval(async function() {"
            "  if (document.hidden) return;"  # 后台空转守卫：全局 pauseTimers 已移除，改由前端自管
            "  try {"
            "    var evs = await window.pywebview.api.pull_events();"
            "    (evs || []).forEach(function(ev) {"
            "      window.__bookan_dispatch(ev.event, ev.data);"
            "    });"
            "  } catch (e) { /* WebView 尚未就绪时静默重试 */ }"
            "}, 1500);"
        ) if android else ""
        js = f"""
        ...
        window.__bookan_is_android = {"true" if android else "false"};
    ...
    # 关闭前保存窗口几何（使用习惯的一部分，仅桌面；Android 无窗口几何）。
    ...
    if android:
        # Android：加载 pywebview 内置 HTTP 服务（http://127.0.0.1:端口）。
        # 明文放行由 manifest 的 usesCleartextTraffic="true" 负责（p4a webview 模板已内置）。
        # 不能加 ssl=True —— cryptography 无法通过 armv7 交叉编译（LONG_BIT 不匹配）。
        webview.start()
    else:

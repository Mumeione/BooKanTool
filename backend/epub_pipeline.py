...
from .config import place_output_file, sanitize_filename
...
class EPUBPipeline:
    ...
    def run(
        ...
        # 先下到私有临时文件再落位：公共目录直写会被分区存储拒绝
        # （重装/清数据后重下同名书刊，旧文件归属旧安装身份 → EACCES）
        stage_fd, stage_path = tempfile.mkstemp(prefix="bookan_epub_", suffix=".epub")
        os.close(stage_fd)
        try:
            ...
            output_path = place_output_file(stage_path, output_path, on_log=log)
        finally:
